...
#Converts numpy arrays into pytorch instances and create a paired datasets

train_loader = DataLoader(train_dataset,batch_size = 32,shuffle=True) 
#batch the dataset in 32 groups  and shuffle the data for training 

class RNNModel(nn.Module):
    def __init__(self, vocab_size ,embedding_dim,hidden_dim,output_dim):
        super(RNNModel, self).__init__()
        self.embedding = nn.Embedding(vocab_size,embedding_dim)
        self.rnn = nn.RNN(embedding_dim,hidden_dim,batch_first=True) #Normal RNN layer
...
def train_rnn(model,train_loader,criterion,optimizer,epochs =5):
    ...
    for epoch in range(epochs):
        epoch_loss = 0 #Cummalative loss is set to 0
